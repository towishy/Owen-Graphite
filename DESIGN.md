...
title: "Owen Graphite Theme Design"
status: adopted
product: "owen-graphite"
surface: "Obsidian workspace, editor, reading view and PDF"
kind: theme
updated: 2026-08-12
...
# Owen Graphite Theme Design

## Decision Summary

| 항목 | 결정 |
| --- | --- |
| 사용자 | Obsidian에서 기술 문서·보고서·지식 archive를 작성하고 출력하는 사용자 |
| Output job | Live Preview에서 작성한 문서를 Reading View와 PDF에서도 같은 hierarchy와 가독성으로 검토한다 |
| Primary object | Markdown document와 Obsidian workspace chrome |
| Primary action | 문서를 작성·검토하고 report-ready PDF로 export한다 |
| Composition | Owen Graphite tokens + opaque document body + selective liquid glass chrome |
| 핵심 판단 | Theme CSS가 canonical visual owner이며 Live Preview·Reading View·PDF parity와 zero-important cascade를 유지한다 |

## Context

Owen Graphite는 standalone app이 아니라 Obsidian theme다. Workspace chrome, typography, tables, code, callouts, plugins와 print surface를 하나의 token·cascade system으로 관리한다.
...
- theme.css
- dev/_order.txt
- dev/scripts/bundle_v3.py
- dev/scripts/dedup_v3.py
...
- Token-first modular CSS cascade
- Document body는 opaque, chrome과 contextual control만 liquid glass
- Korean/Latin typography와 dense table 가독성
- Style Settings en/ko metadata
- Print-specific page break와 header/footer contract

### Excluded Or App-Owned

- !important
- Live Preview·Reading View·PDF 간 별도 visual system
- Plugin 내부 DOM을 theme가 임의로 재구성
- 모든 surface blur

## Design Decisions

### D-01. Zero-important cascade

- **결정**: Import order, layer, specificity와 token으로 cascade를 해결한다
- **이유**: Host·plugin과 장기 호환성을 유지하고 override debt를 만들지 않는다
- **검증**: Bundle에서 !important 0과 cascade harness parity를 검사한다

### D-02. Document surface parity

- **결정**: Heading, table, callout, code와 image가 LP, Reading View, PDF에서 같은 의미 hierarchy를 가진다
- **이유**: 작성 화면과 출력물의 surprise를 줄인다
- **검증**: Canonical document를 세 surface에서 screenshot·PDF 비교한다

### D-03. Theme와 plugin ownership을 분리한다

- **결정**: Theme는 semantic variable과 host chrome을, plugin은 scoped component class를 소유한다
- **이유**: Owen Editor·Exporter와 selector collision을 막는다
- **검증**: Plugin enable/disable matrix와 Style Settings regression을 실행한다
...
- Python CSS bundle and dedup scripts
- zero-important cascade harness
- Live Preview·Reading View·PDF visual matrix
