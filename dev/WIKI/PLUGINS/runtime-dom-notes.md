...
| --- | --- | --- | --- | --- | --- |
| 2026-07-24 | Owen Editor 0.6.27 template preview | `.owen-editor-command-label`, `.owen-editor-command-preview`, `.owen-editor-command-detail-preview` inside `.modal.owen-editor-palette-modal` | Owen Editor owns card geometry; `src/chrome/36-floating-ui-glass-system.css` owns the Graphite typography compatibility override | `dev/TEMP/runtime-evidence/2026-07-24-owen-editor-palette-typography-before.json` and `2026-07-24-palette-notice-after-light.json` / `-after-dark.json` on Obsidian 1.12.7 | Preserve the 13px label and 12px category badge; reduce only the multiline template detail preview to 7.8px, exactly 60% of the label in light and dark modes. |
...
| 2026-07-22 | Owen Editor 0.6.24 palette | `.modal.owen-editor-palette-modal`, `.owen-editor-palette-layout`, `.owen-editor-search`, `.owen-editor-command-section`, `.owen-editor-command-item`, `.owen-editor-favorite-button` | `src/chrome/32-overlay-popover-dataview.css`, `src/chrome/36-floating-ui-glass-system.css`, and `src/surfaces/23-liquid-glass-core.css` own the Graphite modal boundary and generic control opt-out; Owen Editor owns internal palette surfaces | CDP capture in `dev/TEMP/runtime-evidence/2026-07-22-plugin-owen-editor-palette-focus.json` on Obsidian 1.12.7 in `D:\Owen-WIKI` | Outer modal retains Glass; internal layout, sections, cards, favorites, and focused search remain non-blurred and opaque; generic Graphite modal control selectors explicitly yield to the plugin owner |
