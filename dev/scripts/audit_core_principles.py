...
def main() -> int:
    try:
        ...
        run("direct owner guard unit tests", [PYTHON, "dev/scripts/test_direct_owner_guard.py"])
        run("Obsidian sync unit tests", [PYTHON, "dev/scripts/test_sync_obsidian_theme.py"])
        ...
        run("route registry doc freshness", [PYTHON, "dev/scripts/build_route_registry_doc.py", "--check"])
