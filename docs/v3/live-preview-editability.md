...
이 문서는 v3-rewrite가 Live Preview(CodeMirror 6, `.markdown-source-view.mod-cm6`)에서 **클릭→편집 라우팅**을 보존하기 위해 반드시 지켜야 하는 CSS 규칙의 요약입니다.

원본 진실 공급원은 `dev/MAP/cm6-hit-routing-contract.md`(v2.22.99~108 회귀 마라톤 이후 만들어진 contract)입니다. 이 v3 문서는 그 contract를 v3 작업자 관점에서 재정리한 **요약 + 우리 작업의 적용 가이드**입니다.
...
다음은 절대 위반하면 안 됩니다. `scripts/validate_theme.py`가 강제합니다.
...
이 4개 자동 검사가 `scripts/validate_theme.py`의 `live_preview_hit_routing_audit`에 이미 구현되어 있어, v3 작업도 동일 검증을 통과해야 합니다.

## 검증 도구

### C4 보존 계약 (Live Preview hit-routing)

- `scripts/hit_routing_probe.py` (기존) — `dev/test-samples/`에 들어 있는 샘플 vault에서 미리 정의한 좌표를 클릭했을 때 caret이 도달하는 라인을 비교.
- v3 작업 중에는 매 step 종료마다 이 probe를 돌려 baseline(v2.30.14)과 동일한 라인이 나오는지 확인.
...
- 만약 v3에서 새로운 hit-routing 회귀가 발생한다면, 그것은 위 contract가 불완전하다는 증거이므로 **본 문서와 `dev/MAP/cm6-hit-routing-contract.md`를 동시에 업데이트**해야 합니다.
