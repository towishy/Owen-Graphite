...
| Concern | Tool |
| --- | --- |
| Bundle src → dist/theme-v3.css | `scripts/bundle_v3.py` |
| Promote bundle to root `theme.css` | `Copy-Item dist\theme-v3.css theme.css -Force` (Windows) |
| Live Preview hit-routing audit | `scripts/audit_v3_hit_routing.py` |
| Cross-module duplicate selectors | `scripts/v3_audit_duplicate_selectors.py` |
| Computed-style fingerprint | `scripts/capture_computed_fingerprint.py` + `scripts/fp_diff_summary.py` |
| Strip `!important` from src/ | `scripts/v3_strip_important_src.py` (comment-safe) |
| Build release ZIP | `scripts/build_release.py` |
| Sync to Obsidian vault | `scripts/sync_obsidian_theme.py` |
| Pre-commit hook | `scripts/hooks/pre-commit` (bundle + hit-routing) |

## Release procedure for future v3.x.y

1. Edit `src/` only. Never hand-edit `theme.css`.
2. `python scripts/bundle_v3.py` — must end with `OK: bundled ... !important=N` where N is only comment-counts.
3. `python scripts/audit_v3_hit_routing.py` — must be clean.
4. (Optional) re-capture fingerprint to confirm 0 diff against baseline.
5. Bump `manifest.json` version, add `CHANGELOG.md` entry.
6. `python scripts/build_release.py` — emits `dist/Owen-Graphite-<version>.zip`.
