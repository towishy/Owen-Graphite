...
| Glass 강도 | 셀렉트 | Standard | Off / Reduced / Subtle / Standard / Strong |
| Hover 움직임 | 셀렉트 | Standard | Off / Subtle / Standard |
| 한글/CJK +0.5px 보정 | 토글 | ON | 가독성 |

---

## 🖨️ PDF 헤더와 Footer (16종)

| 항목 | 종류 | 기본값 | 설명 |
| ---- | ---- | ------ | ---- |
| **PDF 첫 페이지 Header 표시** | 토글 | ON | 첫 페이지 Header 라벨과 본문 출력 여부 |
| **PDF 첫 페이지 우측 본문** | 텍스트 | (빈 값) | 첫 페이지 우측 상단 본문 |
| **PDF 첫 페이지 우측 라벨** | 텍스트 | (빈 값) | 첫 페이지 우측 상단 라벨 |
| **PDF 첫 페이지 우측 사이드바 색** | 색상 | `#111827` | 우측 수직 막대 색 |
| **PDF 첫 페이지 좌측 본문** | 텍스트 | (빈 값) | 첫 페이지 좌측 상단 본문 |
| **PDF 첫 페이지 좌측 라벨** | 텍스트 | (빈 값) | 첫 페이지 좌측 상단 라벨 |
| **PDF 첫 페이지 좌측 사이드바 색** | 색상 | `#0ea5e9` | 좌측 수직 막대 색 |
| **PDF 첫 페이지 라벨 색** | 색상 | `#6b7280` | 좌/우 라벨 공통 색 |
| **PDF 마지막 페이지 Footer 표시** | 토글 | OFF | 마지막 콘텐츠 뒤에 H3 헤더형 footer notice 표시 |
| **PDF 마지막 페이지 Footer 라벨** | 텍스트 | (빈 값) | 작은 uppercase 라벨 |
| **PDF 마지막 페이지 Footer 제목** | 텍스트 | (빈 값) | footer의 굵은 제목 문구 |
| **PDF 마지막 페이지 Footer 본문** | 텍스트 | (빈 값) | 긴 안내 문구 |
| **PDF 마지막 페이지 Footer 사이드바 색** | 색상 | `#0ea5e9` | 왼쪽 수직 막대 색 |
| **PDF 마지막 페이지 Footer 라벨 색** | 색상 | `#64748b` | 작은 라벨 색 |
| **PDF 마지막 페이지 Footer 제목 색** | 색상 | `#0f172a` | 굵은 제목 색 |
| **PDF 마지막 페이지 Footer 본문 색** | 색상 | `#334155` | 본문 색 |
...
| `ogd-no-h1-number` | YAML cssclasses 또는 body class | H1 Liquid Glass 디자인은 유지하고 number kicker만 숨김 |
