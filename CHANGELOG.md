...
### Fixed
- Kept Mermaid control DOM visible inside Live Preview diagram frames when a Mermaid zoom/pan plugin or Obsidian integration provides controls.
- Positioned Mermaid controls at the top-left inside the diagram frame with Owen Graphite liquid-glass control styling and preserved click hitboxes.

### Added
- Added `dev/test-samples/mermaid-live-preview-zoom-sample.md` with wide Mermaid diagrams and a diagnostic control DOM fixture to confirm whether controls are generated by the host environment.
...
### Fixed
- Windows Obsidian에서 상단 탭과 titlebar 버튼이 정상 표시되도록 검증된 v1.8.66 CSS baseline으로 복원.
- 중간 테스트 빌드에서 만든 Windows chrome 강제 레이아웃, broad `[role="tab"]` selector, 빈 tabbar row 보정 레이어를 제거.

### Changed
- 현재 Style Settings 기준을 실제 CSS와 일치하는 27개 옵션으로 정리.
- 삭제된 중간 테스트 릴리스 기록은 changelog에서 제거하고, 공개 Release는 최신 `v2.22.13`과 안정 기준 `v2.20.1`만 유지.
...
- **기여 섹션** — CONTRIBUTING.md / 당시 `src/README.md` 참조 링크 추가.
...
- **`src/README.md`** — v2.1.0 이후 도입할 모듈 분할 구조, 빌드 파이프라인, 마이그레이션 원칙 정리. 현재는 [dev/README.md](dev/README.md)의 실제 dev 워크플로우로 대체됨.
...
- `theme.css` 실제 모듈 분할은 v2.1.0 부터 단계적으로 진행 예정이었으며, 현재는 [dev/README.md](dev/README.md)의 실제 dev 워크플로우로 대체됨.
